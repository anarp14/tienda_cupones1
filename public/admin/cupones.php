...
<body>
    ...
    $sent = $pdo->query("SELECT * FROM cupones ORDER BY fecha_caducidad");
    ?>
    <div class="container mx-auto">
        ...
        <div class="overflow-x-auto relative mt-4">
            ...
            <button data-modal-toggle="insertar" href="/admin/crear_cupon.php" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-4 py-2 mr-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-900">
                ...
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <th scope="col" class="py-3 px-6">Cupón</th>
                    <th scope="col" class="py-3 px-6">Descuento</th>
                    <th scope="col" class="py-3 px-6">Fecha caducidad</th>
                ...
                <tbody>
                    <?php foreach ($sent as $fila) : ?>
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            <td class="py-4 px-6"><?= hh($fila['cupon']) ?></td>
                            <td class="py-4 px-6"><?= hh($fila['descuento']) ?> %</td>
                            <td class="py-4 px-6"><?= hh($fila['fecha_caducidad']) ?></td>
                            <td class="py-4 px-6">
                                <?php
                                   $hoy = date('Y-m-d'); 
                                   $estado =  ''; 
                                   if($fila['fecha_caducidad'] < $hoy){
                                        $estado = 'Caducado';
                                   } else {
                                    $estado = 'Vigente';
                                   }
                            ...
                            <?php $fila_id = hh($fila['id']) ?>
                                <form action="/admin/editar_cupon.php" method="POST" class="inline">
                                ...
                                <form action="/admin/borrar_cupon.php" method="POST" class="inline">
                                    <input type="hidden" name="id" value="<?= $fila_id ?>">
                                    <button type="submit" onclick="cambiar(event, <?= $fila_id ?>)" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900" data-modal-toggle="borrar">Borrar</button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach ?>
        ...
        <div class="relative p-4 w-full max-w-md h-full md:h-auto">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">¿Seguro que desea borrar este cupón?</h3>
                    <form action="/admin/borrar_cupon.php" method="POST">
        ...
        <div class="relative p-4 w-full max-w-md h-full md:h-auto">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">¿Seguro que desea modificar este cupón?</h3>
                    <form action="/admin/editar_cupon.php" method="POST">
                        <label for="cupon" class="block mb-2 text-sm font-medium">
                            Cupón:
                            <input type="text" name="cupon" id="cupon" class="border text-sm rounded-lg w-full p-2.5">
                        </label>
                        <label for="descuento" class="block mb-2 text-sm font-medium">
                            Descuento:
                            <input type="text" name="descuento" id="descuento" class="border text-sm rounded-lg w-full p-2.5">
                        </label>
                        <label for="fecha_caducidad" class="block mb-2 text-sm font-medium">
                            Fecha de caducidad:
                            <input type="date" name="fecha_caducidad" id="fecha_caducidad" class="border text-sm rounded-lg w-full p-2.5">
                        </label>
        ...
        <div class="relative p-4 w-full max-w-md h-full md:h-auto">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-6 text-center">
                    ...
                    <form action="/admin/crear_cupon.php" method="POST">
                        <label for="cupon" class="block mb-2 text-sm font-medium">
                            Cupón:
                            <input type="text" name="cupon" id="cupon" class="border text-sm rounded-lg w-full p-2.5">
                        </label>
                        ...
                        <input id="ocultoModificar" type="hidden" name="id">
                        <button data-modal-toggle="insertars" type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                            Crear
                        </button>
                    </form>
                </div>
